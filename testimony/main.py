...
def feed(username):
    ...
    if request.method == 'POST':
        ...
        if not tweet:
            ...
            query = 'INSERT INTO tweet ("user_id", "tweet_id", "content") VALUES (:user_id, :tweet_id, :content);'
            query_args = {'user_id': session['user_id'], 'tweet_id': Status.id,'content': tweet}
    ...
    query = """SELECT t.id, t.created, t.content, t.tweet_id
              FROM user AS u
    ...
    tweets = [dict(username=username, id=row[0], created=row[1], content=row[2], tweet_id=row[3]) for row in cursor.fetchall()]
...
def support():
    ...
    status = api.GetReplies(id)

    replies = [s.text for s in status]
    
    return render_template('support.html', replies=replies)
